...
class News extends APP_Controlleri {
	public function index()
	{
		$data = $this->loadDefaultAppConfig();
		$data['sidebarMenu'] = $this->getSidebarMenuStructure('admini/news');
		...
		<script src="assets/scripts/admin/newsi.js"></script>
		';
		$data['indonya'] = base_url('index.php/admini/news');
		$data['engnya'] = base_url('index.php/admin/news');			
	...
	public function newsGetData()
	{
		...
		$data = $this->mnews->getData($page, $row, $order_by, $order, $filter_by, $filter_value);
	...
	public function newsInsertData()
	{
        ...
        else
        {
				...
				$data = $this->mnews->insertData($news);
				
				$this->session->set_flashdata('upload_success', true);
                redirect('admini/news');
        }
	}
	...
	public function newsDeleteData() {
		...
		$this->load->model('admin/mnews');
		$res = $this->mnews->deleteData($_POST['id'], $upload_path, $session_data['username']);
		if ($res) {
			$data['success'] = true;
			$data['msg'] = 'SUCCESS';
		} else {
	...
	public function newsEditData()
	{
        ...
        if ($vval)
        {
        	...
        	if ($change_file) { // delete old file
        		$on = $this->mnews->getDataById($_POST['id']);
			...
			$status = $_POST['status'];
            if ($status == 0){
			$data = $this->mnews->updateData($_POST['id'], $news, $session_data['username']);
			}else{
			$data = $this->mnews->updateData2($_POST['id'], $news, $session_data['username']);
			}
