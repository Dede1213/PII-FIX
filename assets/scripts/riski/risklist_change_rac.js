...
grid.init({
    ...
    dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
        ...
        "ajax": {
            "url": site_url+'/risk/RiskRegister/riskGetRollOver_change_rac/'+username // ajax source
        },
        "columnDefs": [ {
        	"targets": 0,
        	"data": "risk_status",
        	...
        	"render": function ( data, type, full, meta ) {
        		...
        		return '<a target="_self" class="'+cls+'" href="'+site_url+'/maini/mainrac/viewRisk/'+full.risk_id+'">'+data+'</a>';
        	}
        }, {
        	"targets": 7,
        	"data": "risk_status",
...
grid2.init({
    ...
    dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
        ...
        "pageLength": 25, // default record count per page
        "ajax": {
            "url": site_url+'/risk/RiskRegister/riskGetDataUser_change_rac/'+username // ajax source
        },    
        "columnDefs": [ {
        	"targets": 0,
        	"data": "risk_status",
	...
	return {
	        ...
	        submitRiskPeriode2: function() {
	         ...
	         submitRiskPeriode222: function() {
                ...
                mod.find('button.btn-ok-success').one('click', function(){
                    mod.modal('hide');
                    var url = site_url+'/risk/RiskRegister/submitRiskByPeriode2_ignore/'+username;
