	...
	return {
	        ...
	        init: function() {
	            ...
	            $('#input-form-submit-button').click(function(e) {
	                ...
	                if (me.dataMode == 'add') {
	                	var url = site_url+'/user/userInsertData';
	                	var tx = 'Insert';
	                } else {
	                	var url = site_url+'/user/userEditData';
	                	var tx = 'Update';
	                }
	                $.post(
	                	url,
	        ...
	        showInputForm: function() {
	        	$('#input-form')[0].reset();
	        	$('#form-data').find('h4.modal-title').html('Add User');
	        	$('#input-form').find("input[name='username']").prop("readonly", false);
	        	
	        	$('#input-form').find(".help-block").hide();
	        ...
	        deleteData: function(data) {
	        	...
	        	mod.find('button.btn-danger').one('click', function(){
	        		...
	        		var url = site_url+'/user/userDeleteData';
	        		$.post(
	        			url,
	        			{ 'id':  data.DT_RowId},
	        			function( data ) {
